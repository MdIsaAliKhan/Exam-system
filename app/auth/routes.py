...
def get_face_auth_enabled() -> bool:
    try:
        ...
        cur  = conn.cursor()
        cur.execute(
            "SELECT setting_value FROM settings WHERE setting_key='face_auth_enabled'"
        )
        row = cur.fetchone()
        conn.close()
...
def extract_single_encoding(image_array: np.ndarray):
    ...
    return encodings[0].tolist(), None   # return plain list, NOT ndarray
...
def register():
    ...
    if request.method == "POST":
        ...
        face_json      = None   # what gets stored in DB
        ...
        if face_auth_enabled:
            if not face_image_b64:
                ...
                # Library missing — skip silently (dev mode)
                pass
            else:
                try:
                    ...
                    if enc_error:
                        ...
                        # Store as compact JSON string in LONGTEXT column
            ...
            try:
                cur.execute(
                    """INSERT INTO users (name, email, password, role, face_descriptor)
                       VALUES (%s, %s, %s, %s, %s)""",
                    (name, email, password, role, face_json)
                )
                conn.commit()
            except Exception:
...
def login():
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            # Direct login (admin or face auth disabled)
...
def face_verify():
    ...
    error = None
    TOLERANCE = 0.45   # stricter than default 0.6; raise to 0.50 if too strict
    ...
    if request.method == "POST":
        ...
        if not face_image_b64:
            ...
            # Dev mode: skip face check
            _complete_login(pending_id)
            ...
            # ── No face registered yet — allow in but advise ─────────────
